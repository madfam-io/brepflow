...
async function testConstraintSolver() {
  ...
  try {
    ...
    const distanceConstraintId = manager.addConstraint(
    ...
    const perpendicularConstraintId = manager.addConstraint(
      ConstraintType.PERPENDICULAR,
      ['line1', 'line2']
    );
    
    // Add distance constraint: distance between p2 and p3 = 8
    const distance2ConstraintId = manager.addConstraint(
      ConstraintType.DISTANCE,
      ['p2', 'p3'],
      { distance: 8 }
    );
