...
test.describe('Parametric Abacus Studio E2E Tests', () => {
  ...
  test('Creates parametric abacus from scratch in studio', async () => {
    ...
    await test.step('Add linear pattern for rods', async () => {
      ...
      // Add linear pattern
      await page.dragAndDrop(
        '[data-testid="node-linear-pattern"]',
        '[data-testid="node-editor"]',
    ...
    await test.step('Validate 3D viewport visualization', async () => {
      ...
      await expect(page.locator('[data-testid="viewport-3d"]')).toHaveAttribute('data-mode', 'wireframe');
      
      // Back to solid mode
      await page.click('[data-testid="viewport-solid"]');
      await expect(page.locator('[data-testid="viewport-3d"]')).toHaveAttribute('data-mode', 'solid');
    });
  });
...
test.afterEach(async ({ page }, testInfo) => {
  if (testInfo.status === 'failed') {
    ...
    await testInfo.attach('network-logs', { body: JSON.stringify(networkLogs, null, 2), contentType: 'application/json' });
  }
});