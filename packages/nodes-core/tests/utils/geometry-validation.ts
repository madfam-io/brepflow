...
export class GeometryValidator {
  ...
  async validateBasicProperties(shape: ShapeHandle): Promise<{
    ...
    try {
      ...
      if (com.x < bbox.min.x || com.x > bbox.max.x ||
          com.y < bbox.min.y || com.y > bbox.max.y ||
          com.z < bbox.min.z || com.z > bbox.max.z) {
   ...
   */
  async validateManufacturing(shape: ShapeHandle, constraints: {
   ...
   */
  async generateReport(shape: ShapeHandle, name: string): Promise<{
    name: string;
    timestamp: string;
    properties: GeometryProperties;
    topology: any;
