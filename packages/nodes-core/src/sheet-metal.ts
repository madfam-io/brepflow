...
export const EdgeFlangeNode: NodeDefinition<
  ...
  { shape: ShapeHandle },
  { angle: number; length: number; position: 'material-inside' | 'material-outside' | 'bend-outside' }
> = {
...
> = {
  ...
  params: {
    kFactor: { type: 'number', default: 0.44, min: 0, max: 1, description: 'K-Factor for bend allowance' }
  },
  async evaluate(ctx, inputs, params) {
