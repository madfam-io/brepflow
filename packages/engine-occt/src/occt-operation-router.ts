...
import { ProductionWorkerAPI } from './production-worker';
...
export class OCCTOperationRouter implements WorkerAPI {
  ...
  async invoke<T = any>(operation: string, params: any): Promise<T> {
    ...
    const occtOperation = this.operationMap[operation] || operation.toUpperCase().replace(/([A-Z])/g, '_$1').slice(1);
...
export async function createRoutedOCCTWorker(): Promise<WorkerAPI> {
  ...
  try {
    ...
    const baseWorker = await GeometryAPIFactory.getAPI({ forceMode: 'real' });
    ...
    console.log('[OCCTRouter] 📊 Operations mapped:', Object.keys(NODE_TO_OCCT_OPERATION_MAP).length);
...
export function getRoutingStatistics(): {
  ...
  for (const [nodeOp, occtOp] of Object.entries(NODE_TO_OCCT_OPERATION_MAP)) {
    ...
    else if (occtOp.includes('FILLET') || occtOp.includes('CHAMFER') || occtOp.includes('EXTRUDE')) categories.features++;
