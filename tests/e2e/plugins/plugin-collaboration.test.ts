...
test.describe('Plugin Multi-User Collaboration', () => {
  ...
  test('should synchronize plugin states across users', async () => {
    ...
    const session = await pluginHelper.testMultiUserPluginCollaboration({
      ...
      pluginId,
      workflowActions: [
        { user: 'test_user_1', action: 'createNode', data: { type: 'Basic::Box', params: { width: 100 } } },
    ...
    const syncResult = await pluginHelper.validatePluginStateSynchronization(session.sessionId, pluginId);
  ...
  test('should handle concurrent plugin operations', async () => {
    ...
    const session = await pluginHelper.testMultiUserPluginCollaboration({
      ...
      workflowActions: [
        ...
        { user: 'test_user_1', action: 'moveNode', data: { nodeId: 'box1', position: { x: 100, y: 100 } } },
        { user: 'test_user_2', action: 'connectNodes', data: { source: 'sphere1', target: 'box1' } }
      ]
    });
  ...
  test('should handle user disconnection gracefully', async ({ page }) => {
    ...
    await pluginHelper.executePluginAction(pluginId, 'editParameter', { param: 'width', value: 200 });
  ...
  test('should support plugin-specific collaboration features', async ({ page }) => {
    ...
    const annotations = await pluginHelper.executePluginFunction(pluginId, 'getSharedAnnotations', []);
  ...
  test('should handle plugin versioning in collaborative sessions', async () => {
    ...
    const compatibilityCheck = await page.evaluate(() => {
      return (window as any).brepflow?.collaboration?.checkPluginCompatibility?.() || { compatible: false };
    });
  ...
  test('should handle plugin resource sharing between users', async () => {
    ...
    const session = await pluginHelper.testMultiUserPluginCollaboration({
      ...
      pluginId,
      workflowActions: [
        { user: 'test_user_1', action: 'createSharedResource', data: { type: 'material', name: 'Steel' } },
        { user: 'test_user_2', action: 'useSharedResource', data: { name: 'Steel' } }
      ]
    });

    // Verify resource was successfully shared and used
    const sharedResources = await pluginHelper.executePluginFunction(pluginId, 'getSharedResources', []);
